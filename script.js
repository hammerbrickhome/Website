...
// Sticky call duplication fallback (nav-call acts as tel link on click if not anchor)
document.querySelectorAll('.nav-call').forEach(a=>{
  a.addEventListener('click', (e)=>{
    if(a.tagName !== 'A'){ window.location.href='tel:+19295955300'; }
  });
});
...
// Service search filter (works across Services page)
...
async function loadGallery(){
  ...
  try{
    ...
    // Build simple compare pairs based on common filenames
    if(compareRow){
      const pairs = [
...
// Kick off gallery where relevant
